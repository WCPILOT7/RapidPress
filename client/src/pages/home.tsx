...
import { Newspaper, FileText, Users, Send, Share2, Wand2, Eye, Trash2, Upload, Edit, Save, X, ChevronRight, ChevronLeft, Building, User, Calendar, Palette, FileText as FileTextIcon, Quote, Trophy, Languages, Download } from "lucide-react";
...
export default function Home() {
  ...
  const downloadPDF = (release: any) => {
    ...
    // Header with company logo placeholder and PR Studio branding
    doc.setFontSize(8);
    doc.setTextColor(100);
    doc.text("Generated by PR Studio", pageWidth - margin, margin, { align: 'right' });
    ...
    const headlineLines = doc.splitTextToSize(release.headline, maxWidth);
    doc.text(headlineLines, margin, yPosition);
    yPosition += headlineLines.length * 8 + 15;
    
    // Release date and company info
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(60);
    const releaseDate = format(new Date(release.date), "MMMM dd, yyyy");
    doc.text(`${release.company} • ${releaseDate}`, margin, yPosition);
    yPosition += 15;
    
    // Language badge if not English
    if (release.language && release.language !== "English") {
      doc.setFontSize(8);
      doc.setTextColor(100);
      doc.text(`Language: ${release.language}`, margin, yPosition);
      yPosition += 12;
    }
    ...
    for (const line of lines) {
      ...
      if (line.includes('FOR IMMEDIATE RELEASE') || line.includes('###')) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
      } else if (line.startsWith('##') || line.includes('About ')) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(11);
      } else {
        doc.setFont('helvetica', 'normal');
    ...
    doc.setTextColor(80);
    doc.setFont('helvetica', 'normal');
    doc.text("Media Contact:", margin, yPosition);
    yPosition += 8;
    doc.text(release.contact, margin, yPosition);
    yPosition += 6;
    doc.text(release.contactEmail, margin, yPosition);
    yPosition += 6;
    doc.text(release.contactPhone, margin, yPosition);
    
    // Generate filename
    const filename = `${release.company.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_press_release_${format(new Date(release.date), 'yyyy_MM_dd')}.pdf`;
    ...
    toast({
      ...
      description: `Press release saved as ${filename}`,
    });
  };
                  ...
                  {generatedRelease ? (
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          onClick={() => downloadPDF(generatedRelease)}
                          variant="outline"
                        >
